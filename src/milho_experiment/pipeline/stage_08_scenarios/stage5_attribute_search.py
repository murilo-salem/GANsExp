...
def prepare_folds(stage4: Path, out_root: Path, veg: list[str] = ("V8", "V13"),
    ...
    for b in BLOCKS:
        ...
        if (droot / "pairs.csv").is_file():
            ...
            continue
        cmd = [
            sys.executable, "-m", "milho_experiment.pipeline.stage_05_synthesis.stage5_make_pairs",
            "--stage4", str(stage4),
            "--out", str(droot),
            "--veg", *veg, "--rep", *rep,
            "--val-bloco", str(b),
...
def infer_fold(dataroot: Path, ckpt: Path, cfg_out: Path, attr_names: tuple[str, ...],
    ...
    if (infer_dir / "metrics_pairs.csv").is_file():
        ...
        return infer_dir
    cmd = [
        sys.executable, "-m", "milho_experiment.pipeline.stage_05_synthesis.stage5_infer",
        "--dataroot", str(dataroot),
        "--phase", phase,
        "--ckpt", str(ckpt),
        "--out", str(infer_dir),
